	...
	class Mirana_Protocol {
		...
		public static function setMessage($inpVal){
            ...
            if (is_string($inpVal)){
                self::$message = $inpVal;
                $retVal = true;
            } else if (is_array($inpVal)){
				if (isset($inpVal[0])) $t = ["SQLSTATE : ".strVal($inpVal[0])];
				if (isset($inpVal[1])) $t[] = "Error code : ".strVal($inpVal[1]);
				if (isset($inpVal[2])) $t[] = "Error message : ".strVal($inpVal[2]);
				self::$message = implode("; ", $t);
			} else {
				;
			}
            return $retVal;
		...
		public static function handleWebservice($mirana_query_string){
			...
			$retVal = json_encode($result, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE);
			return $retVal;
		}
		...
		public static function parseInputdata(){
			//TODO: rework this: allow data raw : file_get_contents('php://input');
			$retVal = [];
			//parse POST DATA
			if (isset($_POST) && is_array($_POST)){
				...
				foreach ( $_POST as $key => $value ){
					if (is_string($value)){
						if (strlen($value) <= POST_REQUEST_INPUT_LIMIT){
							$retVal[$key] = $value;
						} else {
							;
						}
			...
			if (isset($_GET) && is_array($_GET)){
				foreach ( $_GET as $key => $value ){
					if (!isset($retVal[$key])){
						...
						if (is_string($value)){ //only accepts String(s) for GET
							if (strlen($value) <= GET_REQUEST_INPUT_LIMIT){
								$retVal[$key] = $value;
							} else {
								;
							}
